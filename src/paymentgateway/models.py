...
class PaymentGateway(object):
    ...
    def charge(self, invoice, transaction_ref, **kwargs):
        """
        Method to charge using any medium, payment gateway supports like credit card.

        If payment redirects to their payment page, this method should initiate the transaction
        and return the url to be redirected.

        :param Invoice invoice: Invoice instance to be paid
        :param str transaction_ref: Transaction id where the invoice is related.
        :return response.ChargeResponse: Object of ChargeResponse
        """
        raise NotImplementedError

    def capture(self, transaction_ref, **kwargs):
        """
        Implement this method if payment gateway redirects to their site to charge amount and
        then callback to the system to feed the response. This method is responsible to capture
...
class ChargeResponse(object):
    """
    Helper class for charge response
    """
...
class Postpaid(PaymentGateway):
    """
    Dummy gateway class specially created for the 0 amount payment. 0 amount can be happen if all
